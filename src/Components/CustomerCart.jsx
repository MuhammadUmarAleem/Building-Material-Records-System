  ...
  const [Price, setPrice] = useState(0);
    ...
    useEffect(()=>{
        ...
        async function fetchData() {
          try {
            const response = await fetch(`https://majestic-clever-grapple.glitch.me/GetCustomerCart?UserId=${localStorage.getItem('UserId')}`);
  ...
  return (
              ...
              <MDBRow>
                <MDBCol md="12" lg="3" className="mb-4 mb-lg-0">
                  ...
                  >
                    <MDBCardImage
                      ...
                      className="w-100"
                    />
                    <a href={`/CustomerSpecificProduct?ProductId=${prof.ProductId}`} style={{ color: '#000', textAlign: 'left', textDecoration: 'none'}}>
                      <div
                        className="mask"
                ...
                >
                  ...
                  <div className="d-flex flex-column mt-4">
                    <button type="button" size="sm" className="btn bg-primary btn-block mb-4 mt-20" onClick={()=>{localStorage.setItem("Id", prof.ProductId);localStorage.setItem("Price", prof.UnitPrice);handlesubmit();}}
                     style={{fontSize: '15px'}}>Buy Product</button>
                    <button type="button" size="sm" className="btn bg-warning btn-block mb-4 mt-20"
                     style={{fontSize: '15px'}} onClick={()=>{window.location.href=`https://majestic-clever-grapple.glitch.me/RemoveCustomerCartProduct?UserId=${localStorage.getItem('UserId')}&StockId=${prof.StockId}`}}>Remove From Cart</button>
                  </div>
                </MDBCol>
    ...
    <Modal
          ...
          >
            ...
            <form action={`https://majestic-clever-grapple.glitch.me/CustomerOrderInCart?UserId=${localStorage.getItem('UserId')}&DealerId=${localStorage.getItem('DealerId')}&ProductId=${localStorage.getItem("Id")}&Year=${Address.Year}&Price=${localStorage.getItem("Price")}&Quantity=${Quantity}&StockId=${localStorage.getItem('StockId')}`} method='post' id='CustomerOrder'>
          ...
          <MDBCol className="font-italic mb-1" style={{ textAlign: "right", width: "200px", marginLeft: "auto", marginTop: "10px" }} >
            <button onClick={()=>{submitaddress()}} type="button" className="btn btn-primary btn-block mb-4">Add</button>
          </MDBCol>
          </MDBCardBody>
          </MDBCard>
