...
function CompanyAddProduct() {
  ...
  function handlecategory(event){
    values.prodCategory = event.target.value;
  }
  ...
  const { values, handleBlur, handleChange, handleSubmit, errors, touched } =
    useFormik({
      ...
      onSubmit: (values, action) => {
        document.getElementById('CompanyAddProducts').submit();
      },
    });
      ...
      <MDBRow style={{display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%', marginTop: "5%"}}>
        <MDBCol md='7' >
          ...
          <MDBCardBody>
            ...
            <form enctype='multipart/from-data' action={`https://majestic-clever-grapple.glitch.me/CompanyAddProducts?UserId=${UserId}`} method='post' id='CompanyAddProducts'>
            {
                <img
                  src={selectedImage}
                  alt="Selected"
              ...
              <MDBRow>
              <MDBCol lg='6' className="md-6" onChange={handlecategory}>
              <select className="form-select" wrapperClass='mb-4' label='Product Category' id='prodCategory' name='prodCategory' value={values.prodCategory}
                      onChange={handleChange}
                      onBlur={handleBlur}
                    >
