...
function DealerStock() {
  ...
  useEffect(()=>{
    ...
    async function fetchData() {
      try {
        const response = await fetch(`https://majestic-clever-grapple.glitch.me/GetDealerStock?UserId=${localStorage.getItem("UserId")}`);
  ...
  return (
    ...
    <MDBContainer fluid className="my-5 text-center " >
      ...
      {Product.map((prof,index)=>(
        <MDBCol md="4" lg="3" >
          <MDBCard >
          <a href={`/DealerStockSpecificProduct?ProductId=${prof.ProductId}`}>
            <MDBRipple
              rippleColor="light"
              rippleTag="div"
            ...
            <MDBCardBody>
              ...
              <MDBCol md="6" lg="6" className="mb-3" >
              {checkactive(prof.StatusId)?(<button type="button" onClick={()=>{window.location.href = `https://majestic-clever-grapple.glitch.me/DealerActiveorInActiveStock?StatusId=${prof.StatusId}&ProductId=${prof.ProductId}`}} className="btn bg-danger btn-block mb-4" style={{}}>InActive</button>):<button type="button" onClick={()=>{window.location.href = `https://majestic-clever-grapple.glitch.me/DealerActiveorInActiveStock?StatusId=${prof.StatusId}&ProductId=${prof.ProductId}`}} className="btn bg-success btn-block mb-4" style={{}}>Active</button>}
              </MDBCol>
              </MDBRow>
            </MDBCardBody>
          </MDBCard>
