...
function CompanyThreshold() {
  ...
  useEffect(()=>{
    ...
    async function fetchData() {
      try {
        const response = await fetch(`https://majestic-clever-grapple.glitch.me/GetThresholdCompany?UserId=${localStorage.getItem("UserId")}`);
  ...
  return (
    ...
    <MDBContainer fluid className="my-5 text-center " >
      ...
      {Product.map((prof,index)=>(
        <MDBCol md="4" lg="3" >
          <MDBCard >
          <a href={`/CompanySpecificProduct?ProductId=${prof.ProductId}`} style={{textDecoration:"none",color:"black"}}>
            <MDBRipple
              rippleColor="light"
              rippleTag="div"
            ...
            <a href={`/CompanySpecificProduct?ProductId=${prof.ProductId}`} style={{textDecoration:"none",color:"black"}}>
              ...
              <div className="text-reset">
                <h5>{"Threshold:"+prof.ThresholdQuantity }</h5>
              </div>
              <h4 className="mb-3">{"Available:"+prof.Quantity}</h4>
              </a>
              <MDBRow style={{marginBottom:"-30px"}}>
              <MDBCol md="12" lg="12" className="mb-3">
                <button onClick={()=>{setids(prof.ProductId);setquantity(prof.Quantity);handlestock()}} type="button" className="btn btn-primary btn-block mb-4">Add Stock</button>
              </MDBCol>
              </MDBRow>
            </MDBCardBody>
...
export default CompanyThreshold;
